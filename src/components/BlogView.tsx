...
export const BlogView: React.FC<BlogViewProps> = ({ onSelectEssay, articles = ESSAYS_DATA }) => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-20 space-y-12">
      ...
      {/* Hero Featured Article/News Banner */}
      {featuredPost && (
        <section
        ...
        >
          ...
          <div className="lg:col-span-5 p-6 md:p-8 flex flex-col justify-between bg-[#fcf9f8]">
            <div className="space-y-3">
              ...
              <h2 className="font-display text-2xl md:text-3xl font-bold text-[#1c1b1b] group-hover:text-[#785600] transition-colors leading-snug">
                {featuredPost.title}
              </h2>
            ...
            <div className="pt-6 border-t border-[#d3c4af]/40 mt-6 flex items-center justify-between">
      ...
      {/* Filters & Search Bar */}
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-[#f6f3f2] p-4 rounded-lg border border-[#d3c4af]/50">
        ...
        <div className="flex flex-wrap gap-2 items-center w-full md:w-auto">
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`px-3 py-1 text-xs font-sans font-semibold rounded-full transition-all ${
        ...
        <div className="relative w-full md:w-64">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-1.5 bg-white border border-[#d3c4af] rounded-md font-sans text-xs text-[#1c1b1b] placeholder:text-[#817563] focus:border-[#785600] focus:ring-0"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 space-y-6">
          {filteredPosts.length === 0 ? (
            <div className="text-center py-16 bg-white rounded-lg border border-[#d3c4af]/40 p-8 space-y-3">
              <BookOpen className="w-10 h-10 text-[#817563] mx-auto opacity-40" />
              <h3 className="font-display text-xl font-bold text-[#1c1b1b]">Nenhum publicação encontrada</h3>
              ...
              <button
                ...
                className="px-4 py-2 bg-[#1c1b1b] text-white text-xs font-bold uppercase tracking-widest rounded hover:bg-[#785600] transition-colors mt-2 inline-block"
              >
                Limpar Filtros
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {(gridPosts.length > 0 ? gridPosts : filteredPosts).map((essay) => (
                <article
                  ...
                  className="group cursor-pointer bg-white rounded-lg border border-[#d3c4af]/50 overflow-hidden shadow-2xs hover:border-[#785600] hover:shadow-sm transition-all flex flex-col justify-between"
                >
                  <div>
                    ...
                    <div className="p-5 space-y-2">
                      ...
                      <h3 className="font-display text-xl font-bold text-[#1c1b1b] group-hover:text-[#785600] transition-colors leading-snug">
                        {essay.title}
                      </h3>
        ...
        <aside className="lg:col-span-4 space-y-6">
          {/* Trending / Populares */}
          <div className="bg-white p-6 rounded-lg border border-[#d3c4af]/60 space-y-4">
          ...
          <div className="bg-[#1c1b1b] text-white p-6 rounded-lg space-y-4 relative overflow-hidden">
            ...
            {newsletterSubscribed ? (
              <div className="bg-[#1c5d3a]/20 border border-[#1c5d3a]/40 text-emerald-300 p-3 rounded text-xs flex items-center gap-2 font-semibold">
              ...
              <form onSubmit={handleNewsletterSubmit} className="space-y-2 relative z-10">
                <input
                  ...
                  required
                  className="w-full px-3 py-2 bg-white/10 border border-white/20 text-white rounded text-xs placeholder:text-gray-400 focus:outline-none focus:border-[#f7bd48]"
                />
                <button
                  type="submit"
                  className="w-full py-2 bg-[#785600] hover:bg-[#9a7000] text-white text-xs font-bold uppercase tracking-widest rounded transition-colors"
                >
                  Assinar Boletim
                </button>
              </form>
